...
- **Accounts:** self-hosted username/password sign-in. The server hashes
  ...
  `Authorization: Bearer`. Accounts left over from the earlier Auth0 phase are
  claimed by setting a password on the first sign-in.
...
infrastructure, rotating credentials, or enabling crypto. It contains the Atlas
inventory, Key Vault secret names, two-phase deployment, checks, rollback, and
rotation procedures.
